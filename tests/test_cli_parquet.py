...
class TestParquetOutputShape:
    def test_writes_one_file_per_resource_type(self, tmp_path):
        ...
        # generation-metadata.json and parquet-schema.json are audit sidecars,
        # not FHIR resource files.
        sidecars = {"generation-metadata.json", "parquet-schema.json"}
        assert not [p for p in tmp_path.glob("*.json") if p.name not in sidecars]
